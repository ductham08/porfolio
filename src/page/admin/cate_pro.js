...
const cate_pro_page = {
        async render() {
            const data = await (await fetch("https://porfolio-theta-nine.vercel.app/category_Projects")).json()
            return /*html*/ `
                ...
                <table>
                    <tr>
                        ...
                        <th class="col-md-4 col-4">
                            <button><a href="/admin/categoryproject/add">Add</a></button>
                        </th>
                    </tr>
                    ${data.map((item) => {
                        return /*html*/ `
                        <tr>
                            <td>${item.id}</td>
                            <td><a href="/admin/categoryproject/detail/${item.id}">${item.name}</a></td>
                            <td>
                                <button value="${item.id}" id="btn_remove_post">Remove</button>
                                <button><a href="/admin/categoryproject/edit/${item.id}">Edit</a></button>
                            </td>
                        </tr>
                        `
                    }).join("")}
        ...
        for (const btn of btn_remove) {
            ...
            btn.addEventListener("click", async ()=>{
                ...
                if(confirm){
                    const data = await fetch(`https://porfolio-theta-nine.vercel.app/category_Projects/${id_btn}`,{
                        method:"DELETE"
                    })
                    if(data){
