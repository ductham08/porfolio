...
const home_page = {
        async render() {
            const data_post = await (await fetch("http://localhost:3001/posts?_expand=category_Post&_end=2")).json()
            const data_projects = await (await fetch("http://localhost:3001/projects?_expand=category_Project&_end=3")).json()
            return /*html*/ `
